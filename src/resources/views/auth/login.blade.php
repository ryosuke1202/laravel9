...
<div class="py-6 sm:py-8 lg:py-12">
    <div class="max-w-screen-2xl px-4 md:px-8 mx-auto">
      <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold text-center mb-4 md:mb-8">すでに会員の方</h2>

    <form method="POST" action="{{ route('login') }}" class="max-w-lg border rounded-lg mx-auto">
      @csrf
      <div class="bg-white flex flex-col gap-4 p-4 md:p-8">
        <div>
          <label for="email" class="inline-block text-gray-800 text-sm sm:text-base mb-2">メールアドレス</label>
          <input name="email" type="email" placeholder="メールアドレス" class="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
        </div>

        <div>
          <label for="password" class="inline-block text-gray-800 text-sm sm:text-base mb-2">パスワード　<i class="far fa-eye" id="passEye"></i></label>
          <input name="password" type="password" id="password" placeholder="パスワード" class="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" />
        </div>
        ...
        <div class="flex justify-center items-center relative">
          ...
          <span class="bg-white text-gray-400 text-sm relative px-4">他のサービスでログイン</span>
        </div>
